...
## Finalization
Output generic success message and advise the user to reload Copilot Agent (exit and restart), since anoter process listens to the guide file and reloads the content when it changes. (output verbatim to user: `Please reload Copilot Agent to see the changes.` - This is the only output the user should see.). Terminate the output after this message, no more outputs should be generated.